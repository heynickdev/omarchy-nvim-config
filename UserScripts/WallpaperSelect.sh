...
# This script for selecting wallpapers (SUPER W) - Updated for AWWW (2026)
...
# swww transition config (Awww maintains these flags)
FPS=60
TYPE="any"
DURATION=2
BEZIER=".43,1.19,1,.4"
...
modify_startup_config() {
  ...
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    # Disable Awww/Swww for Video
    sed -i '/^\s*exec-once\s*=\s*\(swww\|awww\)-daemon/s/^/\#/' "$startup_config"
    ...
    # Enable Awww/Swww for Image
    sed -i '/^\s*#\s*exec-once\s*=\s*\(swww\|awww\)-daemon/s/^\#\s*//' "$startup_config"
...
main() {
  ...
  if [[ "$choice" == "$RANDOM_PIC_NAME" ]]; then choice=$(basename "$RANDOM_PIC"); fi

  choice_basename=$(basename "$choice" | sed 's/\(.*\)\.[^.]*$/\1/')
  selected_file=$(find "$wallDIR" -iname "$choice_basename.*" -print -quit)

  if [[ -z "$selected_file" ]]; then exit 1; fi
